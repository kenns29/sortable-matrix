export default function sort2d(matrix) {
  var iter = 0;
  var flag = 'row';
  ...
  var new_id_order;
  while (true) {
    new_id_order = id_order_by_sum(flag, matrix);
    // ++iter;
    if (flag === 'row') {
...
function id_order_by_sum(flag, matrix) {
  var row_id_order = matrix.row_id_order();
  var col_id_order = matrix.col_id_order();
  var sums;
  if (flag === 'row') {
    sums = row_id_order.map(function(row_id, i) {
      ...
      col_id_order.forEach(function(col_id, j) {
        sum += (col_id_order.length - j) * matrix.cell_by_id(row_id, col_id);
      });
    ...
    sums = col_id_order.map(function(col_id, j) {
      ...
      row_id_order.forEach(function(row_id, i) {
        sum += (row_id_order.length - i) * matrix.cell_by_id(row_id, col_id);
      });
