export default function sort2d(matrixData) {
  var iter = 0;
  var flag = "row";
  var row_id_order = matrixData.row_id_order();
  var col_id_order = matrixData.col_id_order();
  var new_id_order;
  while (true) {
    new_id_order = id_order_by_sum(flag, matrixData);
    // ++iter;
    if (flag === "row") {
      if (!same_order(row_id_order, new_id_order) && iter < 10) {
        row_id_order = new_id_order;
        matrixData.order_rows_by_id(row_id_order);
        flag = "col";
      } else break;
    } else if (flag === "col") {
      if (!same_order(col_id_order, new_id_order) && iter < 10) {
        col_id_order = new_id_order;
        matrixData.order_cols_by_id(col_id_order);
        flag = "row";
      } else break;
    }
  }
...
function id_order_by_sum(flag, matrixData) {
  ...
  if (flag === "row") {
    sums = row_id_order.map(function(row_id, i) {
      ...
      col_id_order.forEach(function(col_id, j) {
        sum +=
          (col_id_order.length - j) * matrixData.cell_by_id(row_id, col_id);
      });
    ...
    sums = col_id_order.map(function(col_id, j) {
      ...
      row_id_order.forEach(function(row_id, i) {
        sum +=
          (row_id_order.length - i) * matrixData.cell_by_id(row_id, col_id);
      });
